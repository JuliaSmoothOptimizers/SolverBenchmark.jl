...
function test_bmark()
  @testset "Testing bmark" begin
    ...
    stats = solve_problems(DummySolver, problems)
    @test stats isa DataFrame
    stats = solve_problems(DummySolver, problems, reset_problem=false)
    stats = solve_problems(DummySolver, problems, reset_problem=true)

    solvers = Dict(:dummy => DummySolver, :dummy => DummySolver)
  ...
  @testset "Testing logging" begin
    ...
    with_logger(ConsoleLogger()) do
      ...
      solve_problems(DummySolver, nlps)
      reset!.(nlps)

      # @info "Testing logger with specific columns on `solve_problems`"
      # solve_problems(DummySolver, nlps, colstats=[:name, :nvar, :elapsed_time, :objective, :dual_feas])
      # reset!.(nlps)

      # @info "Testing logger with hdr_override on `solve_problems`"
      # hdr_override = Dict(:dual_feas => "‖∇L(x)‖", :primal_feas => "‖c(x)‖")
      # solve_problems(DummySolver, nlps, info_hdr_override=hdr_override)
      # reset!.(nlps)
    end
  end
end
